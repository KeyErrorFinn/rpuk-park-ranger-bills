...
logInputCopyOutputBtn.addEventListener("click", () => {
    ...
    if (sheetInputText == "") {
        ...
        setTimeout(() => {
            logInputCopyOutputBtn.innerHTML = '<i class="fa-classic fa-regular fa-copy fa-fw"></i> Output';
        ...
        listOfBills.appendChild(billTab);
            ...
            billTabHeader.addEventListener("click", () => {
                ...
                if (billTab.classList.contains("open")) {
                    ...
                    billTabInfo.style.maxHeight = fullHeight;
                } else {
            ...
            for (const itemName in personItems) {
                ...
                billTabInfo.appendChild(billInfoItem);
                    ...
                    // Creates BILL INFO ITEM HEADER \\
                    const billInfoItemHeader = document.createElement("div");
                    billInfoItemHeader.classList.add("bill-info-item-header");
                    // Adds BILL INFO ITEM HEADER to BILL INFO ITEM
                    billInfoItem.appendChild(billInfoItemHeader);


                        // Creates BILL INFO ITEM HEADER TITLE \\
                        const billInfoItemHeaderTitle = document.createElement("div");
                        billInfoItemHeaderTitle.classList.add("bill-info-item-header-title");
                        const titleQuantity = itemInfo["Taken"][0]-itemInfo["Given"][0];
                        let titleQuantityText = "";
                        if (titleQuantity !== 0 && itemInfo["Taken"][0] > 0 && itemInfo["Given"][0] > 0) {
                            if (titleQuantity < 0) {
                                titleQuantityText = ` <b>-x${titleQuantity * -1}</b>`;
                            } else {
                                titleQuantityText = ` <b>x${titleQuantity}</b>`;
                            }
                        }
                        billInfoItemHeaderTitle.innerHTML = `${itemName}${titleQuantityText}`;
                        // Adds BILL INFO ITEM HEADER TITLE to BILL INFO ITEM HEADER
                        billInfoItemHeader.appendChild(billInfoItemHeaderTitle);
